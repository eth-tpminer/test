...
turn_on() {
    ...
    if [ ! -f "$PATH_TURN_ON_SH" ];then
        ...
        echo '  echo "stminer已启动"' >> $PATH_TURN_ON_SH
        echo 'else' >> $PATH_TURN_ON_SH
        echo '  if [ $COUNT -ne 0 ]; then' >> $PATH_TURN_ON_SH
        echo '      echo "stminer已启动, 无需重复启动"' >> $PATH_TURN_ON_SH
        echo '  elif [ $(id -u) -ne 0 ]; then' >> $PATH_TURN_ON_SH
        echo '      echo "使用ROOT用户登录才能启动stminer"' >> $PATH_TURN_ON_SH
...
installapp() {
    ...
    colorEcho ${GREEN} "开始安装stminer-V-${VERSION}"
    ...
    wget -P $PATH_KT "${DOWNLOAD_HOST}/stminer_v${VERSION}_linux" -O "${PATH_KT}/${PATH_EXEC}" 1>/dev/null

    filterResult $? "拉取程序 stminer_v${VERSION}_linux"
