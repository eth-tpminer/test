...
PATH_CACHE="/root/stminer/.cache"

PATH_LICENSE="/root/stminer/license"

PATH_CONFIG="/root/stminer/.env"

PATH_NOHUP="/root/stminer/nohup.out"
PATH_ERR="/root/stminer/err.log"


PATH_TURN_ON="/etc/profile.d"
PATH_TURN_ON_SH="/etc/profile.d/stm.sh"
...
stop() {
    colorEcho $BLUE "终止STMinerProxy进程"
    killall stminer
    sleep 1
}
...
start() {
    ...
    if [ $? -eq 1 ]; then
        ...
        nohup "${PATH_ST}/${PATH_EXEC}" 2>err.log &
        # nohup "${PATH_ST}/${PATH_EXEC}" >/dev/null 2>log &
        filterResult $? "启动程序"

        # getConfig "ST_START_PORT"
...
turn_on() {
    ...
    if [ ! -f "$PATH_TURN_ON_SH" ];then
        ...
        echo "  cd ${PATH_ST}" >> $PATH_TURN_ON_SH
        echo "  nohup "${PATH_ST}/${PATH_EXEC}" 2>err.log &" >> $PATH_TURN_ON_SH
...
installapp() {
    ...
    checkProcess "stminer"
    if [ $? -eq 1 ]; then
        colorEcho ${RED} "发现正在运行的STMinerProxy, 需要停止才可继续安装。"
        colorEcho ${YELLOW} "输入1停止正在运行的STMinerProxy并且继续安装, 输入2取消安装。"
    ...
    if [[ ! -f $PATH_CONFIG ]];then
        setConfig ST_START_PORT $((RANDOM%65535+1))
    fi

    colorEcho $BLUE "拉取程序"
    # wget -P $PATH_ST "${DOWNLOAD_HOST}/${ORIGIN_EXEC}" -O "${PATH_ST}/${PATH_EXEC}" 1>/dev/null
    wget -P $PATH_ST "${DOWNLOAD_HOST}/stproxy_v${VERSION}_linux" -O "${PATH_ST}/${PATH_EXEC}" 1>/dev/null
...
check_hub() {
    ...
    tail -f /root/stminer/nohup.out
}

check_err() {
    colorEcho ${YELLOW} "按住CTRL+C后台运行"
    tail -f /root/stminer/err.log
}
...
set_port() {
    ...
    setConfig ST_START_PORT $choose
